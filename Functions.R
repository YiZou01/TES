...
TES <- function(x,knots=40){
  TESab <- function (x,knots=40,method=c("a","b"))
  {
    ...
    parameter='Weibull'
    tryCatch(
      {md <- nls(value ~ SSweibull(Logm, Asym, Drop, lrc, pwr),data=result)        
    ...
    if(Error_four) #If an error accur for four prarmeter model, then run three parameter model
    {
      parameter='logistic'
      tryCatch({md <- nls(value~SSlogis(Logm, Asym, xmid, scal),data= result)
      Coe <- summary(md)$coefficients
      a <- Coe[1,1]
  ...
  tbl[3, 2] <- round(sqrt(tbl[1, 2] ^ 2 + tbl[2, 2] ^ 2), 2)
